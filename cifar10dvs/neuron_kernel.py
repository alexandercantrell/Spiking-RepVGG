...
class ParametricConnectingLIFNodeFPTTKernel(NeuronFPTTKernel):
    ...
    def neuronal_charge(self) -> str:
        ...
        codes += cfunction.mul(z=f'{self.dtype} LIFNodeFPTTKernel_temp_var2',x='s_seq[t]', y='LIFNodeFPTTKernel_temp_var', dtype=self.dtype)
        codes += cfunction.sub(z='LIFNodeFPTTKernel_temp_var',x='LIFNodeFPTTKernel_temp_var',y='LIFNodeFPTTKernel_temp_var2',dtype=self.dtype)
        if self.decay_input:
        ...
        codes += cfunction.sub(z=f'{self.dtype} LIFNodeFPTTKernel_temp_var3',x='v_th',y='v_v_seq[t]',dtype=self.dtype)
        codes += cfunction.mul(z='LIFNodeFPTTKernel_temp_var3',x='LIFNodeFPTTKernel_temp_var3',y='s_seq[t]',dtype=self.dtype)
        codes += cfunction.add(z='LIFNodeFPTTKernel_temp_var',x='LIFNodeFPTTKernel_temp_var',y='LIFNodeFPTTKernel_temp_var3',dtype=self.dtype)
...
class ParametricConnectingLIFNodeBPTTKernel(NeuronBPTTKernel):
    ...
    def post_core(self):
        codes = base.CodeTyper(16)
        codes.append(cfunction.mul(z='grad_v_init[index]', x='grad_h', y='grad_v_init[index]', dtype=self.dtype))
    ...
    def core(self):
        ...
        #
        core_codes.append(cfunction.mul(z=f'grad_h', x='grad_h', y='grad_v_init[index]', dtype=self.dtype))
        ...
        core_codes.append(cfunction.sub(z=f'{self.dtype} grad_h_next_to_v', x=cfunction.constant(None, x=1., dtype=self.dtype), y='decay[0]', dtype=self.dtype))
        core_codes.append(cfunction.mul(z='grad_v_init[index]', x='grad_h_next_to_v', y='s_seq[t]', dtype=self.dtype))
        core_codes.append(cfunction.sub(z='grad_v_init[index]', x='grad_h_next_to_v', y='grad_v_init[index]', dtype=self.dtype))
        ...
        if self.hard_reset:
            core_codes.append(cfunction.sub(z=f'{self.dtype} grad_h_to_s', x='v_v_seq[t]', y='v_reset', dtype=self.dtype))
        else:
            core_codes.append(f'{self.dtype} grad_h_to_s = v_v_seq[t];')
        core_codes.append(cfunction.mul(z='grad_h_to_s', x='grad_h_to_s', y='decay[0]', dtype=self.dtype))
        core_codes.append(cfunction.add(z='grad_h_to_s', x='grad_h_to_s', y='v_th', dtype=self.dtype))
        core_codes.append(cfunction.sub(z='grad_h_to_s', x='grad_h_to_s', y ='v_v_seq[t]', dtype=self.dtype))
        core_codes.append(cfunction.mul(z='grad_s_seq[t]', x='grad_h_to_s', y='grad_h', dtype=self.dtype))

        with base.CodeBlock(core_codes):
            if self.decay_input: #computes the inside of the decay mult as a shortcut for the gradient
                core_codes.append(cfunction.sub(z=f'{self.dtype} temp_var', x='v_th', y='v_v_seq[t]', dtype=self.dtype))
                core_codes.append(cfunction.mul(z='temp_var', x='temp_var', y='s_seq[t]', dtype=self.dtype))
                core_codes.append(cfunction.add(z='temp_var', x='temp_var', y='v_v_seq[t]', dtype=self.dtype))
                core_codes.append(cfunction.sub(z='temp_var', x='h_seq[t]', y='temp_var', dtype=self.dtype))
                ...
                if self.hard_reset:
                    core_codes.append(cfunction.sub(z=f'{self.dtype} temp_var', x='v_reset', y='v_v_seq[t]', dtype=self.dtype))
                    core_codes.append(cfunction.mul(z=f'{self.dtype} temp_var2', x='s_seq[t]', y='temp_var', dtype=self.dtype))
                    core_codes.append(cfunction.sub(z='temp_var', x='temp_var', y='temp_var2', dtype=self.dtype))
                    core_codes.append(cfunction.mul(z='temp_var', x='temp_var', y='grad_h', dtype=self.dtype))
                else:
                    core_codes.append(cfunction.mul(z=f'{self.dtype} temp_var', x='s_seq[t]', y='v_v_seq[t]', dtype=self.dtype))
                    core_codes.append(cfunction.sub(z='temp_var', x='v_v_seq[t]', y='temp_var', dtype=self.dtype))
                    core_codes.append(cfunction.mul(z='temp_var', x='temp_var', y='grad_h', dtype=self.dtype))
...
class SpikingParametricConnectingLIFNodeFPTTKernel(NeuronFPTTKernel):
    def __init__(self, decay_input: bool, hard_reset: bool, dtype: str):
        super().__init__(hard_reset, dtype)
        self.decay_input = decay_input
        self.add_param(ctype=f'const {dtype} *', cname='s_seq')
...
class SpikingParametricConnectingLIFNodeBPTTKernel(NeuronBPTTKernel):
    def __init__(self, decay_input: bool, surrogate_function: Callable, hard_reset: bool, detach_reset: bool, dtype: str):
        super().__init__(surrogate_function, hard_reset, detach_reset, dtype)
        self.decay_input = decay_input
        self.add_param(ctype=f'const {dtype} *', cname='decay')
...
class SpikingParametricConnectingLIFNodeATGF(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x_seq: torch.Tensor, s_seq:torch.Tensor, v_init: torch.Tensor, v_th: float, v_reset: float or None, decay: torch.Tensor, forward_kernel: SpikingParametricConnectingLIFNodeFPTTKernel, backward_kernel: SpikingParametricConnectingLIFNodeBPTTKernel):
        if x_seq.dtype == torch.float16 and v_init.numel() % 2 != 0:
            raise ValueError('When using the the PLIF neuron with half2 cupy backend, the numer of neurons should be even to avoid the wrong gradient of tau caused by padding!')
        py_dict = {
            'x_seq': x_seq,
