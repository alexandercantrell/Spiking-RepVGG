...
class ParaConnLIFNode(BaseNode):
    ...
    circumvent the problem of applying Spiking Resnet to Parametric Leaky Integrate-and-Fire (LIF) Neurons. In
    addition, it should also help solve the exploding gradient problem present in traditional Spiking Resnet.
    ...
    def neuronal_charge(self, x: torch.Tensor, y: torch.Tensor):
        if self.decay_input:
            if self.v_reset is None or self.v_reset == 0:
                inner_id = y*self.v
                outer_id = y * (self.v_threshold-self.v)
                self.v = self.v + outer_id + (x - self.v + inner_id) * self.w.sigmoid()
            else:
                v_sub_v_reset = self.v - self.v_reset
                inner_id = y * v_sub_v_reset
                outer_id = y * (self.v_threshold - self.v)
                self.v = self.v + outer_id + (x - v_sub_v_reset + inner_id) * self.w.sigmoid()
        else:
            if self.v_reset is None or self.v_reset == 0:
                inner_id = y * self.v
                outer_id = y * self.v_threshold - self.v
                self.v = self.v + outer_id + (inner_id - self.v) * self.w.sigmoid() + x
            else:
                v_sub_v_reset = self.v - self.v_reset
                inner_id = y * v_sub_v_reset
                outer_id = y * self.v_threshold - self.v
                self.v = self.v + outer_id - (inner_id - v_sub_v_reset) * self.w.sigmoid() + x
...
class SpikeParaConnLIFNode(BaseNode):
    ...
    def multi_step_forward(self, x_seq: torch.Tensor, y_seq: torch.Tensor):
        if self.backend == 'torch':
            ...
            if self.forward_kernel is None or not self.forward_kernel.check_attributes(hard_reset=hard_reset,
                                                                                       dtype=dtype,
                                                                                       decay_input=self.decay_input):
                self.forward_kernel = neuron_kernel.SpikingParametricConnectingLIFNodeFPTTKernel(decay_input=self.decay_input,
                                                                                   hard_reset=hard_reset, dtype=dtype)
            if self.backward_kernel is None or not self.backward_kernel.check_attributes(
                    surrogate_function=self.surrogate_function.cuda_codes, hard_reset=hard_reset,
                    detach_reset=self.detach_reset, dtype=dtype, decay_input=self.decay_input):
                self.backward_kernel = neuron_kernel.SpikingParametricConnectingLIFNodeBPTTKernel(decay_input=self.decay_input,
                                                                                    surrogate_function=self.surrogate_function.cuda_codes,
                                                                                    hard_reset=hard_reset,
                                                                                    detach_reset=self.detach_reset,
                                                                                    dtype=dtype)
            ...
            spike_seq, v_seq = neuron_kernel.SpikingParametricConnectingLIFNodeATGF.apply(
                x_seq.flatten(1),y_seq.flatten(1),self.v.flatten(0),self.v_threshold,self.v_reset,self.w.sigmoid().to(x_seq),
                self.forward_kernel, self.backward_kernel)
